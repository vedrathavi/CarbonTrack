...
export async function getLatest(req, res) {
  try {
    ...
    const insights = await Insight.find({ homeId: resolved }).sort({ generatedAt: -1, createdAt: -1 }).limit(limit).lean();
...
export async function generateForHome(req, res) {
  try {
    ...
    const saved = await insightService.generateAndStoreInsights(resolved, payload);
