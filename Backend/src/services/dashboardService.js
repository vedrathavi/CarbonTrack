...
async function resolveHomeId(identifier) {
  ...
  if (mongoose.Types.ObjectId.isValid(asString)) return new mongoose.Types.ObjectId(asString);
...
function perApplianceTotals(emissions) {
  const result = {};
  const entries = emissions instanceof Map ? Array.from(emissions.entries()) : Object.entries(emissions || {});
  for (const [k, arr] of entries) {
    const sum = Array.isArray(arr) ? arr.reduce((a, b) => a + (Number(b) || 0), 0) : 0;
...
export async function getTodayForHome(homeId, date = null) {
  ...
  const doc = await HourlyEmission.findOne({ homeId: resolved, date: target }).lean();
  if (!doc) return null;
...
export async function getRangeForHome(homeId, days = 7, endDate = null) {
  ...
  const docs = await HourlyEmission.find({ homeId: resolved, date: { $gte: start, $lte: end } })
    .sort({ date: 1 })
    .lean();
  ...
  for (let i = 0; i < days; i++) {
    ...
    const d = map[key];
    if (d) {
      const total = Number((d.summary?.totalEmissions ?? (Array.isArray(d.totalHourly) ? d.totalHourly.reduce((a,b)=>a+(Number(b)||0),0) : 0)).toFixed(2));
      out.push({ date: d.date, total, topAppliance: d.summary?.topAppliance || null });
    } else {
...
export async function getComparison(homeId, days = 7, endDate = null) {
  ...
  const homeDocs = await HourlyEmission.find({ homeId: resolved, date: { $gte: start, $lte: end } }).lean();
  ...
  for (const d of homeDocs) {
    ...
    const total = Number(d.summary?.totalEmissions ?? (Array.isArray(d.totalHourly) ? d.totalHourly.reduce((a, b) => a + (Number(b) || 0), 0) : 0)) || 0;
  ...
  const allDocs = await HourlyEmission.find({ date: { $gte: start, $lte: end } }).lean();
  const totalsByDate = {};
  for (let i = 0; i < days; i++) {
    const d = new Date(start);
    d.setUTCDate(start.getUTCDate() + i);
