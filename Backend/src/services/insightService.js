...
function buildCandidatesFromDashboard({ today, week, month, comparison } = {}) {
  ...
  try {
    if (today && today.summary) {
      c.push({ title: `Today's total: ${Number(today.summary.totalEmissions || 0).toFixed(1)} g` });
      if (today.topAppliance) c.push({ title: `Top appliance today: ${today.topAppliance}` });
    }
    if (week && Array.isArray(week.data)) {
      c.push({ title: `Week total: ${week.total || week.data.reduce((s,d)=>s+Number(d.total||0),0)} g` });
    }
    if (month && Array.isArray(month.data)) {
      c.push({ title: `Month daily average: ${month.avg?.toFixed ? month.avg.toFixed(1) : (month.avg||0)} g` });
    }
    if (comparison) {
      c.push({ title: `Compared to global avg: home ${comparison.homeAvg} g vs global ${comparison.globalAvg} g` });
    }
  } catch (e) {
    // best-effort
  }
...
export async function generateAndStoreInsights(homeId, dashboardPayload = {}) {
  ...
  try {
    ...
    const recent = await HourlyEmission.find({ homeId }).sort({ date: -1 }).limit(7).lean();
    homeContext = {
      ...
      recentSummary: recent.length ? `Latest total ${Number(recent[0].summary?.totalEmissions||0).toFixed(1)} g, top: ${recent[0].summary?.topAppliance||'unknown'}` : "no recent data",
  ...
  const prompt = [];
  prompt.push("You are an assistant that writes short, specific, human-friendly energy insights for a single-home dashboard. Output MUST be a JSON array only.");
  prompt.push("Respond ONLY with a JSON array of insight objects. Do not include any additional explanation.");
  prompt.push("Insight object shape: { id: string, type: 'anomaly'|'improvement'|'milestone'|'suggestion'|'curiosity', title: string, description: string, impactLevel?: 'low'|'medium'|'high', tags?: string[] }");
  if (homeContext) {
    prompt.push(`Home: ${homeContext.name || ''}`);
    if (homeContext.address) {
      const { city, state, country } = homeContext.address || {};
      prompt.push(`Location: ${city || ''}${city? ', ' : ''}${state || ''}${state? ', ' : ''}${country || ''}`);
    }
  ...
  prompt.push('Produce up to 10 concise insights as the JSON array described above. Keep tone friendly, specific, sometimes witty. Avoid dates or extra commentary.');
  // Additional instruction to avoid vague or non-actionable language
  prompt.push('Do NOT produce vague calls-to-action or filler phrases like "Let\'s see how much we can trim next", "Let\'s try", "Consider trying", or generic encouragements without specifics. Each insight MUST include at least one concrete, actionable item or measurable estimate (for example: "Increase thermostat by 2°C to save ~8% on cooling" or "Run laundry after 9pm to shift 1 kWh off-peak").');
  // Brevity rules: force short titles and descriptions for UI fit
  prompt.push('Brevity constraints: Titles MUST be <= 8 words. Descriptions MUST be 25 words or fewer and no more than 2 short sentences. Use terse actionable language (e.g. "Raise thermostat 2°C — ~8% cooling save"). Do NOT add extra explanation.');
  ...
  let rawOutput = null;
  try {
    rawOutput = await gemini.generateTextWithSDK('gemini-2.5-flash', promptText);
  } catch (e) {
  ...
  let insights = null;
  if (!rawOutput) {
    console.error('[insightService] No output from Gemini SDK. This likely means the AI service is not configured or unavailable.');
  ...
  try {
    ...
    if (start !== -1 && end !== -1 && end > start) {
      try {
        ...
        console.error('[insightService] Failed to parse JSON from Gemini output. Raw output:', text);
  ...
  try {
    ...
    console.info('[insightService] Dropped legacy unique index homeId_1_type_1_generatedAt_1');
  } catch (e) {
  ...
  for (const ins of (insights || []).slice(0, 10)) {
    try {
      const sourceSeed = ins.id || ins.title || ins.description || JSON.stringify(ins || {});
      const sourceId = crypto.createHash('sha1').update(String(sourceSeed)).digest('hex').slice(0, 16);
      ...
      const doc = {
        ...
        tags: ins.tags || [],
        generatedAt: ins.generatedAt ? normalizeDateToUTCDay(ins.generatedAt) : normalizedDate,
      };

      // Upsert by homeId + sourceId + generatedAt to avoid duplicates while allowing multiple
      // insights of the same `type` per day.
      const filter = { homeId: doc.homeId, sourceId: doc.sourceId, generatedAt: doc.generatedAt };
      const updated = await Insight.findOneAndUpdate(filter, { $set: doc }, { upsert: true, new: true, setDefaultsOnInsert: true });
      saved.push(updated);
    } catch (e) {
