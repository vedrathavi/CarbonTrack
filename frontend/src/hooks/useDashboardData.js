...
import { getDashboardToday, getDashboardWeek, getDashboardMonth, getDashboardComparison, getSimulationLatestRoute, getSimulationHourlyRoute } from "../utils/constants";
...
function normalizeHourlyArray(arr) {
  ...
  for (let i = 0; i < Math.min(24, arr.length); i++) out[i] = Number(arr[i] || 0);
  return out;
}
...
export default function useDashboardData(homeId) {
    ...
    async function fetchToday() {
      try {
        ...
        if (res?.data?.data) {
          ...
          try {
            const SOCKET_URL = import.meta.env.VITE_API_WS_URL || window.location.origin;
            if (!socketRef.current) {
              const s = io(SOCKET_URL, { withCredentials: true });
              socketRef.current = s;
              s.on("connect", () => {
                if (resolvedHomeRef.current) s.emit("joinHome", { homeId: resolvedHomeRef.current });
              });
              s.on("hourly-emission-update", (payload) => {
                if (!payload) return;
                if (!resolvedHomeRef.current || String(payload.homeId) !== String(resolvedHomeRef.current)) return;
                // full-day payload
                if (payload.fullDay) {
                ...
                setToday((prev) => {
                  if (!prev) return prev;
                  const next = { ...prev };
                  next.totalHourly = normalizeHourlyArray(next.totalHourly || []);
                  const idx = Number(payload.hour);
                  if (!Number.isNaN(idx) && idx >= 0 && idx < 24) {
                    next.totalHourly = next.totalHourly.map((v, i) => (i === idx ? Number(payload.total || 0) : v));
                  }
                  if (payload.perAppliance) {
                    ...
                    for (const [k, v] of Object.entries(payload.perAppliance)) {
                      const arr = Array.isArray(next.emissions[k]) ? [...next.emissions[k]] : new Array(24).fill(0);
                      if (!Number.isNaN(idx) && idx >= 0 && idx < 24) arr[idx] = Number(v || 0);
                      next.emissions[k] = arr;
                      next.applianceTotals[k] = Number((arr.reduce((a,b)=>a+(Number(b)||0),0)).toFixed(2));
                    }
                  }
                  next.summary = next.summary || {};
                  next.summary.totalEmissions = Number((next.totalHourly.reduce((a,b)=>a+(Number(b)||0),0)).toFixed(2));
                  return next;
                });
              });
            } else if (socketRef.current && resolvedHomeRef.current) {
              socketRef.current.emit("joinHome", { homeId: resolvedHomeRef.current });
            }
          } catch {
        ...
        try {
          ...
          if (latest?.data?.doc) {
            ...
            const normalized = {
              ...
              summary: { totalEmissions: doc.summary?.totalEmissions || 0, topAppliance: doc.summary?.topAppliance || null },
              applianceTotals: {},
            ...
            if (doc.homeId) {
              try {
                ...
                if (!socketRef.current) {
                  const s = io(SOCKET_URL, { withCredentials: true });
                  socketRef.current = s;
                  s.on("connect", () => s.emit("joinHome", { homeId: String(doc.homeId) }));
                  s.on("hourly-emission-update", () => {});
                } else {
                  socketRef.current.emit("joinHome", { homeId: String(doc.homeId) });
                }
        ...
        try {
          ...
          if (hourly?.data) {
            ...
            if (typeof d.hour === "number") {
              ...
              setToday({ date: d.date, totalHourly: arr, summary: { totalEmissions: arr.reduce((a,b)=>a+(Number(b)||0),0) }, applianceTotals: {}, emissions });
              setLoading(false);
              return;
            }
          }
  ...
  const fetchWeek = useCallback(async function fetchWeek() {
    if (!homeId) return;
    setLoading(true);
    try {
      ...
      setWeek(null);
      setError(err?.response?.data?.message || err.message || "Failed to load week data");
    } finally { setLoading(false); }
  }, [homeId]);

  const fetchMonth = useCallback(async function fetchMonth() {
    if (!homeId) return;
    setLoading(true);
    try {
      ...
      setMonth(null);
      setError(err?.response?.data?.message || err.message || "Failed to load month data");
    } finally { setLoading(false); }
  }, [homeId]);

  const fetchComparison = useCallback(async function fetchComparison(days = 7) {
    if (!homeId) return;
    setLoading(true);
    try {
